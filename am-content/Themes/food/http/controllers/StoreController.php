...
class StoreController extends controller
{
	...
	public function show($slug)
	{
		...
		$currency_name = Options::where('key','currency_name')->first()->value;
		
		if($store && $store->role_id == 3 && $store->status == 'approved' || $store->status == 'offline') {
			$galleries = explode(",", $store->gallery->content);
			return view('theme::store.index',compact('store','galleries','five_rattings','four_rattings','three_rattings','two_rattings','one_rattings','currency_name'));
		}else{
			return abort(404);
		}
	}

	public function store_data($slug)
	{
